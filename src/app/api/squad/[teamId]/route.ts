...
export async function GET(
  ...
  if (directKey) {
    ...
    console.error(
      "[squad] No API key env set (APIFOOTBALL_KEY or RAPIDAPI_KEY)."
    );
  ...
  try {
    ...
    const playersLen = Array.isArray(data?.response)
      ? data.response?.[0]?.players?.length ?? 0
      : 0;

    console.log(
      `[squad] squads team=${teamId} players=${playersLen}`,
      planErr ? `(err: ${planErr})` : ""
    );
    ...
    if (!p1.res.ok) {
      console.warn(
        "[squad] players fallback upstream error",
        first?.errors
      );
      return NextResponse.json(
        { error: "Upstream error", ...first },
        { status: p1.res.status }
      );
    }

    // Collect pages if any (keep it light for free plan)
    const totalPages = Number(first?.paging?.total ?? 1) || 1;
    const allPlayers: any[] = Array.isArray(first?.response)
      ? first.response
      : [];
    ...
    // Normalize players -> { id, name, position, number }
    const normalized = allPlayers
      .map((r: any) => ({
        id: r.player?.id,
        name: r.player?.name,
        position:
          r.statistics?.[0]?.games?.position ??
          r.player?.position ??
          null,
    ...
    console.log(
      `[squad] fallback players team=${teamId} season=${season} players=${normalized.length}`
    );
